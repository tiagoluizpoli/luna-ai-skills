...
for ((i=1; i<=MAX_ITERATIONS; i++)); do
  ...
  codex exec --dangerously-bypass-approvals-and-sandbox "Start the Ralph Loop iteration $i. Read .plan/RULES.md FIRST (canonical engineering rules), then agents.local.md, then .plan/PRD.md, then .plan/index.md (single source of truth for epics and tasks), then the current epic and task files. Pick the next task, complete ONLY that task, then log your status to .plan/progress.txt. Recent commits: $recent_commits" | tee "$tmpfile"
