...
async function selectGroupedConflicts(conflictingItems) {
  ...
  function render() {
    ...
    if (cursor < startIndex) {
      startIndex = cursor;
    } else if (cursor >= startIndex + maxViewportHeight) {
      startIndex = cursor - maxViewportHeight + 1;
    }
    ...
    // Top scroll indicator (reserved line to maintain constant line height)
    if (startIndex > 0) {
    ...
    for (let idx = 0; idx < maxViewportHeight; idx++) {
    ...
    // Bottom scroll indicator (reserved line to maintain constant line height)
    if (startIndex + maxViewportHeight < lines.length) {
      output += `│  \x1b[36m▼\x1b[0m\n`;
    } else {
      output += `│\n`;
    }
